...
```{r echo=FALSE, message=FALSE, layout='l-body'}
...
ggplotly(plt.NYC.OTP) %>% 
  layout(title = 'Subway cars are getting worse at arriving at their destination on schedule.', 
...
ggplotly(plt.NYC.SWA) %>% 
  layout(title = 'Subway cars are arriving with a less routine timing between them.',
  titlefont = list(size = 17)) %>%
  config(displayModeBar = FALSE) 
```
...
```{r echo=FALSE, warning=FALSE, layout='l-screen-inset', fig.height=5.5}
...
conclude with hypothesis of whether L train shutdown has increased accidents 
Change drag behavior when plotly is updated to 1.43
