...
ggplotly(plt.NYC.OTP) %>% 
  layout(title = 'Subway cars getting worse at arriving at their destination on schedule.', 
...
ggplotly(plt.NYC.SWA) %>% 
  layout(title = 'Subway cars arriving with less routine timing between them.',
  titlefont = list(size = 16)) %>%
  config(displayModeBar = FALSE) 
```
